...
public class FastMath {
	...
	double rinv = sqrt(d2); // rinv ? 1.0 / hypot(x, y)
	x *= rinv; // x ? cos ?
	...
	// Find f (a first approximation of ?) from the LUT
	double f = ASIN_TAB[ind];
	double cf = COS_TAB[ind]; // cos(f)

	// sin(f) == ind / 256.0
	// Note that sf is truncated, hence not identical to y.
	double sf = yp - FRAC_BIAS;
	double sd = y * cf - x * sf; // sin(?-f) = sin? cosf - cos? sinf
